...
1. Сами виртуалки. Скачиваем и устанавливаем  [VirtualBox](https://www.virtualbox.org/)
...
10. Если не открылось проверяем последовательно несколько мест:
- Настройки сгенерированной virtualbox. Там должен быть включен и настроен проброс портов
- в  файле `Homestead.yaml`  настройки `folders` соответствуют реально существующим папкам
- в файле `host` есть запись которая соединяет ip виртуалки и домен из `Homestead.yaml`

12. Подключение к запущенной виртуалке через  SSH / SFTP
```yaml
...
13. Проверить конфиг ngins внутри виртуалки
```
vagrant ssh
